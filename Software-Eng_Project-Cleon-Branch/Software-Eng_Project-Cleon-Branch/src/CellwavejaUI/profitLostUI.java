	...
	public profitLostUI() {
		...
		btnCalculate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (profit_Loss(textField.getText(),textField_1.getText())==0){
					lblresult.setText("Product was not sold ");
				}else {
					lblresult.setText(String.valueOf(profit_Loss(textField.getText(),textField_1.getText())));
				}
			}
		});
	 ...
	 public  float profit_Loss(String model,String name) {
			float costPrice=0;
			float pL = 0;
			for (int x=0;x<addproductinformationGUI.newProducts.size();x++) {
				String m = addproductinformationGUI.newProducts.get(x).getModelNumber();
				String p= addproductinformationGUI.newProducts.get(x).getProductName();
				if (m.equals(model) && p.equals(name) ) {
					costPrice=addproductinformationGUI.newProducts.get(x).getCostPrice();
				}
			}
			for ( int i =0;i<AddnewTransactiontofile.newTransactions.size(); i++) {
				String m = AddnewTransactiontofile.newTransactions.get(i).getProductModelNumber();
				String p= AddnewTransactiontofile.newTransactions.get(i).getProductName();
				if (m.equals(model) && p.equals(name) ) {
					pL +=  AddnewTransactiontofile.newTransactions.get(i).getCashtendered()- costPrice;
				} 
			}
			return pL;
		}
